...
public class Fisherman extends Human implements Fishing, Eater {
    ...
    //НЕТ, ВЫ ПОНИМАЕТЕ????? ПОЛЕ ОБЪЕКТА-ЭКЗЕМПЛЯР ДРУГОГО ОБЪЕКТА

    public Fisherman(String name, int skill, int kindness, int love, int hunger) {
        super(name);
        this.skill = skill;
